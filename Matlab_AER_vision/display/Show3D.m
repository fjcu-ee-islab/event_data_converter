...
for i=1:length(q)
    ...
    plot3(TD.ts(TD.p == q(i))./1e3, TD.x(TD.p == q(i)), abs(-(TD.y(TD.p == q(i)))), '.', 'color', cc(i,:))
    hold on
end
...
if length(varargin) > 1
    ...
    for i = 1:length(Tracker)
        plot3(Tracker{i}.X, Tracker{i}.Y, Tracker{i}.T/1e3, '.', 'color', cc(i,:))
    end
end
grid on
grid minor
ylabel('Pixel X address')
zlabel('Pixel Y address')
